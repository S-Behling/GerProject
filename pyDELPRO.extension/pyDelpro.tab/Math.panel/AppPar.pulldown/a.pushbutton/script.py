...
def RelatorioInputs(IputVal, par, cat, parRest, valRest):
    ...
    print("O valor  >" + str(IputVal)) 
    print("<  foi aplicado ao parametro  >" + par) 
    print( "<  em todos os(as)  >" + cat)
    if YesNo == "s":
...
msg = "qual o parametro que voce deseja somar a um valor?"
parametroGet = raw_input(msg)
parametroGet = confereInput(parametroGet, 3, msg)
print("> "+parametroGet)
valor = raw_input("digite o valor a ser somado")
print("> " + valor)
msg = "Onde voce deseja colocar esse valor?"
...
for j in ElesCategoria:
    ...
    parametro = j.LookupParameter(parametroGet)
    parametro2 = j.LookupParameter(parametroEdit)
    atual = parametro.AsValueString()
    atual = int(atual)
    valorfinal = valor + float(atual)
    ...
    if (str(parametro2)) != "None":
        if str(parametro.Definition.Name) != str(parametroEdit):
            ...
            valorfinal = ajustaTipo(parametro2, valorfinal)
            parametro2.Set(valorfinal)
